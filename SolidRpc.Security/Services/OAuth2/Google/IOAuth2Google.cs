...
namespace SolidRpc.Security.Services.OAuth2.Google {
    ...
    public interface IOAuth2Google {
        ...
        /// </summary>
        /// <param name="client_id">The client ID string that you obtain from the API Console, as described in Obtain OAuth 2.0 credentials.</param>
        /// <param name="response_type">The client ID string that you obtain from the API Console, as described in Obtain OAuth 2.0 credentials.</param>
        ...
        /// <param name="login_hint">can be the user's email address or the sub string, which is equivalent to the user's Google ID. If you do not provide a login_hint and the user is currently logged in, the consent screen includes a request for approval to release the user’s email address to your app</param>
        /// <param name="include_granted_scopes">If this is provided with the value true, and the authorization request is granted, the authorization will include any previous authorizations granted to this user/application combination for other scopes; see Incremental Authorization. Note that you cannot do incremental authorization with the Installed App flow.</param>
        /// <param name="access_type">The allowed values are offline and online. The effect is documented in Offline Access; if an access token is being requested, the client does not receive a refresh token unless offline is specified.</param>
        /// <param name="openid.realm">openid.realm is a parameter from the OpenID 2.0 protocol, not from OAuth 2.0. It is used in OpenID 2.0 requests to signify the URL-space for which an authentication request is valid. Use openid.realm if you are migrating an existing application from OpenID 2.0 to OpenID Connect.</param>
        ...
        Task Authorize(
            string client_id,
            IEnumerable<string> response_type,
            ...
            bool include_granted_scopes = default(bool),
            string access_type = default(string),
